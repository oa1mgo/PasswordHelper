...
public class AddActivity extends Activity {
	...
	private boolean getData() {
		...
		try {
			byte[] bname  =mname.getBytes("UTF-8");
			byte[] bkeydata = {1,2,3,4,5,6,7,8};
			byte[] tmp = DESUtil.desEncrypt(bname, bkeydata);
			String result = DESUtil.parseByte2HexStr(tmp);			
			tmp = DESUtil.parseHexStr2Byte(result);
			tmp = DESUtil.desDecrypt(tmp, bkeydata);
			mname = new String(tmp);
